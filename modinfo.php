<!--     ce fichier permet de modifie une information de section INFORMATION PERSONNELLE         -->
<?php include 'connection.php';?>
  <?php    if(isset($_GET['modinfo'])){
  $modifier_info=$_GET['modinfo'];
...
$sql= "UPDATE `cv`.`info_pesonnelle` SET `info` = '$aa' WHERE `info_pesonnelle`.`id` = $modifier_info";
...
echo "bien";
       ...
       $sql="SELECT * FROM `info_pesonnelle` WHERE id=$modifier_info";
          $result = mysqli_query($conn, $sql);
           $row = mysqli_fetch_assoc($result);?>

           <div class="w3-container w3-card w3-white w3-margin-bottom">
        <h2 class="w3-text-grey w3-padding-16"><i class="fa fa-edit fa-fw w3-margin-right w3-xxlarge w3-text-orange"></i>update information personnelle</h2>
   
        <form class="w3-container w3-card-4" action="" method="Post">
  <h2 class="w3-text-blue">Input Form</h2>
    <p>Ajouter text sans appostrophe ou accent .</p>
  <p>      
  <label class="w3-text-blue"><b>information </b></label>
  <input class="w3-input w3-border" name="nname" type="text" value="<?php
   echo $row['info'];?>"></p>
       
  <button class="w3-btn w3-orange" name='modInfo'>Modifeir</button></p>
</form>
      </div>
