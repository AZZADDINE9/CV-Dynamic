<!--     ce fichier permet de modifie une information de section LANGUE         -->
<?php include 'connection.php';?>
  <?php    if(isset($_GET['modl'])){
  $modifierL=$_GET['modl'];
...
if(isset($_POST['modLangues'])){
$aa=$_POST['nname'];

$bb=$_POST['level'] ;
//$sql="INSERT INTO `skills` (`id`,`nonSK`,`levelSK`) VALUES (NULL,'$a','$b')";
$sql= "UPDATE `cv`.`languages` SET `langues` = '$aa', `niveau` = '$bb' WHERE `languages`.`id` = $modifierL";
...
echo "bien";
       ...
       $sql="SELECT * FROM `languages` WHERE id=$modifierL";
          $result = mysqli_query($conn, $sql);
           $row = mysqli_fetch_assoc($result);?>

           <div class="w3-container w3-card w3-white w3-margin-bottom">
        <h2 class="w3-text-grey w3-padding-16"><i class="fa fa-edit fa-fw w3-margin-right w3-xxlarge w3-text-orange"></i>update Langue</h2>
   
        <form class="w3-container w3-card-4" action="" method="Post">
  <h2 class="w3-text-blue">Input Form</h2>
  <p>      
  <label class="w3-text-blue"><b>Langue Name</b></label>
  <input class="w3-input w3-border" name="nname" type="text" value="<?php
   echo $row['langues'];?>"></p>
  <p>      
  <label class="w3-text-blue"><b>Langue level</b></label>
  <input class="w3-input w3-border" name="level" value="<?php echo $row['niveau'];?>"type="number"></p>
  <p>      
  <button class="w3-btn w3-orange" name='modLangues'>Modifeir</button></p>
</form>
      </div>
